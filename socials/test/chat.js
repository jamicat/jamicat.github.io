...
ensureMessageHoverActions(row) {
    ...
    for (const [value, key] of quickReactions) {
        const button =
            document.createElement("button");
...
openReactionPicker(
    ...
    const picker =
        document.createElement("div");
    picker.className =
        "jami-reaction-picker terminal2 theme-body";

    const addChoice = reaction => {
        const button =
            document.createElement("button");
        button.type = "button";
        button.className =
        ...
        if (reaction.kind === "custom") {
            const image =
                document.createElement("img");
            image.src = reaction.src;
            image.alt = reaction.label;
            image.className =
                "jami-reaction-picker-image";
            button.appendChild(image);
        } else {
            button.textContent = reaction.value;
        }
...
jumpToReplyTarget(type, id) {
    ...
    const target = this.messages.querySelector(selector);
    if (!target) return;
